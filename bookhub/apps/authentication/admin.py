...
from .models import AuthToken, Session, LoginAttempt


@admin.register(AuthToken)
class AuthTokenAdmin(admin.ModelAdmin):
    """Админка для токенов"""
    list_display = ('user', 'token_type', 'is_expired', 'is_blacklisted', 'created_at', 'expires_at')
    ...
    fieldsets = (
        ('Основная информация', {
            'fields': ('user', 'token_type', 'is_blacklisted')
        }),
        ('Детали токена', {
...
class SessionAdmin(admin.ModelAdmin):
    """Админка для сессий"""
    list_display = ('user', 'session_key_short', 'ip_address', 'last_activity', 'is_expired')
    ...
    fieldsets = (
        ...
        ('Статус', {
            'fields': ('last_activity', 'expires_at', 'is_expired')
        }),
        ('Временные метки', {
            'fields': ('created_at', 'updated_at')
        }),
...
class LoginAttemptAdmin(admin.ModelAdmin):
    ...
    fieldsets = (
        ('Основная информация', {
            'fields': ('email', 'ip_address', 'success')
        }),
        ('Детали попытки', {
            'fields': ('failure_reason', 'user_agent')
        }),
        ('Временные метки', {
            'fields': ('created_at', 'updated_at')
        }),
